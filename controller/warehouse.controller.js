    ...
    try {
        ...
        if (partType === 'Warehouse') {
            ...
            for (let rowIndex = 2; rowIndex <= worksheet.actualRowCount; rowIndex++) {
                ...
                const insertedDocument = await WareHouse.create(document);
...
export const viewWareHouse = async (req, res, next) => {
    try {
        ...
        return (warehouse.length > 0) ? res.status(200).json({ WareHouse: warehouse, status: true }) : res.status(404).json({ error: "Not Found", status: false })
    }
    catch (err) {
        console.log(err);
        return res.status(500).json({ error: "Internal Server Error", status: false });
