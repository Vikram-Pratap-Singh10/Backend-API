...
import { Dealer } from "../model/part.model.js";
    ...
    try {
        ...
        if (partType === 'Dealers') {
            ...
            for (let rowIndex = 2; rowIndex <= worksheet.actualRowCount; rowIndex++) {
              ...
              const insertedDocument = await Dealer.create(document);
...
export const viewDealer = async (req, res, next) => {
    try {
        let dealer = await Dealer.find();
        return (dealer.length >0) ? res.status(200).json({ Dealer: dealer, status: true }) : res.status(404).json({ error: "Not Found", status: false })
    }
    catch (err) {
        console.log(err);
        return res.status(500).json({ error: "Internal Server Error", status: false });
